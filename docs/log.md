...
- **2026-09-10** — First upload of 2.6.0+12 rejected: AAB signed with `holy-quran.keystore` (Feb 2025) but Play expected the 2020 `alquran.jks` upload key, which is lost with the old Windows laptop. Play App Signing is enabled, so an **upload key reset** was requested with the new key's certificate. Details in [platforms/android.md](platforms/android.md#signing).
- **2026-09-10** — Web build fixed and redeployed to GitHub Pages: base href `/quran/`, single bootstrap in `index.html`, no default JSON content type on GET (CORS preflight), sequential juz preload + 429 retry (API limit 12 req/s), hash URL strategy, `scripts/deploy_web.sh`. See [platforms/web.md](platforms/web.md).
